...
# 전국 시/군/구 상세 좌표 딕셔너리 (250여 개 시군구 완벽 매핑)
REGION_COORDS = {
    # 서울특별시
    '강남구': (37.5172, 127.0473), '강동구': (37.5301, 127.1238), '강북구': (37.6396, 127.0257), '강서구': (37.5509, 126.8495),
    '관악구': (37.4784, 126.9516), '광진구': (37.5385, 127.0823), '구로구': (37.4954, 126.8874), '금천구': (37.4568, 126.8952),
    '노원구': (37.6542, 127.0568), '도봉구': (37.6688, 127.0471), '동대문구': (37.5744, 127.0400), '동작구': (37.5124, 126.9393),
    '마포구': (37.5663, 126.9016), '서대문구': (37.5791, 126.9368), '서초구': (37.4837, 127.0324), '성동구': (37.5635, 127.0365),
    '성북구': (37.5894, 127.0167), '송파구': (37.5145, 127.1060), '양천구': (37.5169, 126.8665), '영등포구': (37.5263, 126.8962),
    '용산구': (37.5326, 126.9900), '은평구': (37.6027, 126.9291), '종로구': (37.5730, 126.9794), '중구': (37.5641, 126.9979), '중랑구': (37.6066, 127.0927),
    # 경기도
    ...
    '가평군': (37.8315, 127.5095), '연천군': (38.0964, 127.0748),
    # 인천광역시
    '중구': (37.4738, 126.6216), '동구': (37.4738, 126.6433), '미추홀구': (37.4635, 126.6502), '연수구': (37.4101, 126.6783),
    '남동구': (37.4469, 126.7315), '부평구': (37.5070, 126.7218), '계양구': (37.5374, 126.7377), '서구': (37.5454, 126.6760),
    '강화군': (37.7465, 126.4880), '옹진군': (37.4659, 126.6285),
    # 충청북도 / 충청남도 / 대전 / 세종
    '청주시': (36.6424, 127.4890), '충주시': (36.9910, 127.9259), '제천시': (37.1326, 128.2141), '음성군': (36.9324, 127.6908),
    '진천군': (36.8553, 127.4355), '천안시': (36.8151, 127.1139), '공주시': (36.4465, 127.1190), '보령시': (36.3332, 126.6129),
    '아산시': (36.7898, 127.0018), '서산시': (36.7845, 126.4503), '논산시': (36.1872, 127.0987), '당진시': (36.8897, 126.6459),
    '대전': (36.3504, 127.3845), '세종': (36.4800, 127.2890),
    # 기타 광역시 및 도
    '부산': (35.1796, 129.0756), '대구': (35.8714, 128.6014), '광주': (35.1595, 126.8526), '울산': (35.5384, 129.3114),
    '전주시': (35.8242, 127.1480), '포항시': (36.0190, 129.3435), '창원시': (35.2280, 128.6811), '제주': (33.4996, 126.5312)
}
...
def get_fast_coordinates(addr):
    ...
    # 2. 광역 지자체 기본값 fallback
    if '서울' in addr: return 37.5665, 126.9780
    if '경기' in addr: return 37.4138, 127.5183
    if '인천' in addr: return 37.4563, 126.7052
    if '충북' in addr or '충청북도' in addr: return 36.6357, 127.4912
    if '충남' in addr or '충청남도' in addr: return 36.5184, 126.8000
...
with tab2:
    ...
    for _, row in top_facilities.iterrows():
        ...
        # 출발지와 처리장 좌표가 다를 때만 동선 생성
        if (target_lat != f_lat) or (target_lng != f_lng):
            folium.PolyLine(
